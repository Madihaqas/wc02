...
\begin{parts}
  ...
  \part How many different nonograms can be made from a $5\times5$ grid without any clues?
  \begin{solution}
    A box can have only two possible options here, either the box will be colored or left blank. Since there are 25 boxes in total, the number of possibilities raised to the power of the total number of boxes is the answer.\\
    $2^{25}=33554432$  nonograms can be made from a 5 × 5 grid without any clues.
  \end{solution}

  \part How many different nonograms can be made from a $5\times5$ grid with clues?
  \begin{solution}
    
If the clues are given, then by following those clues, only one pattern is possible. For example, in the nonograms given in part a, all the students who followed the instructions correctly, reached the exact same answer.
  \end{solution}
\end{parts}
